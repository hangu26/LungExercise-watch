...
## 🔗 휴대폰 앱과 연동 조건
워치 앱과 휴대폰 앱(`LungExercise`)이 정상적으로 연동되려면 아래 조건이 반드시 충족되어야 합니다:

1. **Application ID 동일**  
   - 휴대폰 앱(`pulmoguide`)과 워치 앱의 `applicationId`는 동일해야 합니다.  
   - 현재 값:  
     ```
     kr.daejeonuinversity.lungexercise
     ```
...
✅ 이 두 조건이 만족되어야 Wear OS 앱과 Android 휴대폰 앱 간의 데이터 통신 및 연동이 정상적으로 동작합니다.
...
- 해당 앱(pulmoguide 또는 LungExerciseWatch): ✅ 허용  
